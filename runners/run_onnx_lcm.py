...
class RKNN2Model:
    ...
    def __init__(self, model_dir):
        ...
        self.inference_time = 0
        # timing for the end-of-run summary
        self.load_time = load_time
        self.compute_time = 0.0   # accumulated pure-inference time across all calls
        self.num_calls = 0
    ...
    def __call__(self, **kwargs) -> List[np.ndarray]:
        ...
        self.compute_time += time.time() - _t
        self.num_calls += 1
        results_list = []
        for res in results:
...
class RKNN2StableDiffusionPipeline(DiffusionPipeline):
    ...
    def _encode_prompt(
        ...
        if do_classifier_free_guidance and negative_prompt_embeds is None:
            ...
            negative_prompt_embeds = self.text_encoder(input_ids=uncond_input.input_ids.astype(np.int64))[0]
    ...
    def prepare_latents(self, batch_size, num_channels_latents, height, width, dtype, generator, latents=None):
        ...
        # numpy>=2 (NEP 50): float32 array * np.float64 scalar promotes to float64; the
        # unet ONNX wants float32 -> cast back. (Only bites under numpy 2; vendor uses <2.)
        latents = (latents * np.float64(self.scheduler.init_noise_sigma)).astype(dtype)
    ...
    def __call__(
        ...
        # Guidance-scale embedding: only 4-input distilled LCM unets take a `timestep_cond`
        # (time_cond_proj_dim=256). 3-input LCM-LoRA-merged unets (time_cond_proj_dim=None)
        # have no such input, so we skip the w-embedding and run a single forward pass.
        time_cond_proj_dim = self.unet.config.get("time_cond_proj_dim")
        if time_cond_proj_dim is not None:
            w = np.full(bs, guidance_scale - 1, dtype=prompt_embeds.dtype)
            w_embedding = self.get_guidance_scale_embedding(
                w, embedding_dim=time_cond_proj_dim, dtype=prompt_embeds.dtype
            )
        else:
            w_embedding = None

        # Adapted from diffusers to extend it for other runtimes than ORT.
        # optimum exports the unet with a FLOAT timestep (the vendor's hand-made ONNX used
        # int64). Match our optimum export.
        timestep_dtype = np.float32

        num_warmup_steps = len(timesteps) - num_inference_steps * self.scheduler.order
        for i, t in enumerate(self.progress_bar(timesteps)):
            # Our unet ONNX takes a rank-1 [1] timestep (optimum exports a 0-d scalar, but
            # RKNN can't accept scalar inputs, so we graph-patch the unet to [1] + squeeze;
            # see sd_conversion_for_onnx_and_rknn_insights.md). Feed [t], not scalar.
            timestep = np.array([t], dtype=timestep_dtype)
            # kwargs order = the unet's input order: sample, timestep,
            # encoder_hidden_states[, timestep_cond]. Drop timestep_cond for 3-input models.
            unet_inputs = dict(
                sample=latents,
                timestep=timestep,
            ...
            if w_embedding is not None:
                unet_inputs["timestep_cond"] = w_embedding
    ...
    def get_guidance_scale_embedding(self, w, embedding_dim=512, dtype=None):
        ...
        # numpy>=2 (NEP 50): np.log(10000.0) is a float64 scalar, promoting the float32
        # math above to float64. The unet ONNX wants float32 -> cast back.
...
def print_timing_summary(pipe, generate_time, save_time):
    """Per-component load + compute time, plus overall wall time (seconds)."""
    comps = [("text_encoder", pipe.text_encoder),
             ("unet", pipe.unet),
             ("vae_decoder", pipe.vae_decoder)]
    load_total = sum(c.load_time for _, c in comps)
    compute_total = sum(c.compute_time for _, c in comps)
    print("\n=== timing summary (seconds) ===")
    print(f"{'component':<14}{'load':>9}{'compute':>10}{'calls':>7}")
    for name, c in comps:
        print(f"{name:<14}{c.load_time:>9.2f}{c.compute_time:>10.2f}{c.num_calls:>7}")
    print("-" * 40)
    print(f"{'load total':<14}{load_total:>9.2f}")
    print(f"{'compute total':<14}{'':>9}{compute_total:>10.2f}  (pure model inference)")
    print(f"{'generate':<14}{'':>9}{generate_time:>10.2f}  (compute + scheduler/CPU + tokenize)")
    print(f"{'image save':<14}{'':>9}{save_time:>10.2f}")
    print(f"{'overall':<14}{'':>9}{load_total + generate_time + save_time:>10.2f}  (load + generate + save)")


def main(args):
    logger.info(f"Setting random seed to {args.seed}")
    # Seed torch's global RNG too: LCMScheduler.step() samples per-step noise via torch
    # (generator=None), so without this the denoise noise is unseeded and reruns differ even
    # with a fixed --seed (the numpy generator below only seeds the *initial* latents).
    ...
    # Use the tokenizer shipped in the model dir if present; else download from the Hub.
    # (Avoids a ~30s offline stall on the Pi retrying huggingface.co.)
    local_tokenizer = os.path.join(args.i, "tokenizer")
    tokenizer = CLIPTokenizer.from_pretrained(
        local_tokenizer if os.path.isdir(local_tokenizer) else "openai/clip-vit-base-patch16")

    # VAE decoder: default to the model's own vae_decoder, or an alternate (e.g. TAESD) dir.
    vae_dir = args.vae_dir if args.vae_dir else os.path.join(args.i, "vae_decoder")
    if args.vae_dir:
        logger.info(f"Using alternate VAE decoder: {vae_dir}")
    ...
    pipe = RKNN2StableDiffusionPipeline(
        ...
        vae_decoder=RKNN2Model(vae_dir),
    ...
    # Resolve output path. -o (or --output-file) is treated as an EXACT file if it ends in an
    # image extension; otherwise it's a directory and the filename is auto-generated.
    target = args.output_file or args.o
    if not target:
        raise SystemExit("Provide -o <path> (image file or dir) or --output-file <path>.")
    if os.path.splitext(target)[1].lower() in {".png", ".jpg", ".jpeg", ".webp", ".bmp"}:
        out_path = target
    ...
    print_timing_summary(pipe, generate_time, save_time)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-o", default=None,
                        help="Output directory (filename auto-generated). Not needed if "
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="Optional path to an alternate VAE decoder dir (e.g. a TAESD decoder) to use "
             "instead of <model>/vae_decoder. Must contain the model + config.json. The "
             "runner unscales latents by config['scaling_factor'] before decoding, so the "
             "alternate VAE's config must set scaling_factor to match its exported graph "
             "(SD VAE uses 0.18215; a TAESD export that bakes its own unscaling into the "
             "graph should set 1.0). One TAESD decoder works across all SD1.5 LCM models "
             "(shared latent space).")
    parser.add_argument(
        "--output-file",
        default=None,
        type=str,
        help="Exact output image path to write. If unspecified, a filename is auto-generated "
             "under -o from the prompt/seed/steps (get_image_path).")
