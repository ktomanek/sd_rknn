...
class RKNN2LatentConsistencyPipeline(DiffusionPipeline):
    ...
    def prepare_latents(self, batch_size, num_channels_latents, height, width, dtype, generator, latents=None):
        ...
        # unet wants float32 -> cast back. (This board's inference venv is numpy 2.x.)
    ...
    def __call__(
        ...
        # Our unet was exported by optimum with a FLOAT timestep (the vendor's hand-made
        # ONNX used int64); the RKNN model inherits float. Feed float32, not int64.
        # (input_ids stays int32 above — RKNN wants int32 there.)
        ...
        for i, t in enumerate(self.progress_bar(timesteps)):
            ...
            # kwargs order = the unet's input order (RKNN2Model feeds inputs positionally):
            # sample, timestep, encoder_hidden_states[, timestep_cond].
            unet_inputs = dict(
                sample=latents,
                timestep=timestep,
    ...
    def get_guidance_scale_embedding(self, w, embedding_dim=512, dtype=None):
        ...
        # math above to float64. The unet wants float32 -> cast back.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=None,
        type=str,
        help="Optional path to an alternate VAE decoder RKNN dir (e.g. a TAESD decoder) to "
             "use instead of <model>/vae_decoder. Must contain model.rknn + config.json. The "
